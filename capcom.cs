...
namespace CAPCOM
{
    ...
    public class CAPCOMPlugin : MonoBehaviour
    {
        ...
        public static void ReloadServerWithModel(string modelFileName)
        {
            ...
            string serverExe = Path.GetFullPath(Path.Combine(kspRoot, "CAPCOM_Server", "llama-server.exe"));
            string modelPath = Path.GetFullPath(Path.Combine(kspRoot, "GameData", "CAPCOM", "Models", modelFileName));

            if (!File.Exists(serverExe))
            {
                UnityEngine.Debug.LogWarning("[CAPCOM] llama-server.exe missing in CAPCOM_Server/");
                return;
            }

            if (!File.Exists(modelPath))
            {
                UnityEngine.Debug.LogWarning("[CAPCOM] Model file not found: " + modelPath);
                return;
            }
            ...
            try
            {
                ...
                UnityEngine.Debug.Log("[CAPCOM] Local AI Server started with: " + modelFileName);
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError("[CAPCOM] Failed to start server: " + ex.Message);
            }
        }
        ...
        public static void UnloadModel()
        {
            if (serverProcess != null && !serverProcess.HasExited)
            {
                try
                {
                    ...
                    serverProcess = null;
                    UnityEngine.Debug.Log("[CAPCOM] llama-server process cleanly terminated.");
                }
                catch { }
            }
                ...
                try
                {
                    ...
                    foreach (var dir in dirs)
                    {
                        ...
                        if (dirName.Equals("Squad", StringComparison.OrdinalIgnoreCase) ||
                            ...
                            dirName.Equals("CAPCOM", StringComparison.OrdinalIgnoreCase))
                            continue;
    ...
    public static class ChatStorage
    {
        ...
        {
            string folder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "GameData", "CAPCOM", "SavedChats");
        ...
        public static List<ChatSessionData> LoadAllSessions()
        {
            ...
            foreach (var file in files)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("[CAPCOM] Error reading chat session: " + ex.Message);
                }
            }
            return list;
        }
        ...
        public static void SaveSession(ChatSessionData session)
        {
            ...
            {
                UnityEngine.Debug.LogError("[CAPCOM] Failed to save chat session: " + ex.Message);
            }
        }
    ...
    public class ModelCard
    {
        ...
        {
            string kspRoot = AppDomain.CurrentDomain.BaseDirectory;
            string folder = Path.Combine(kspRoot, "GameData", "CAPCOM", "Models");
            if (!Directory.Exists(folder)) Directory.CreateDirectory(folder);
        ...
        public static async Task DownloadModelAsync(ModelCard card)
        {
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    request.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) KSPCAPCOM/1.0";
                    ...
                    {
                        CAPCOMPlugin.ReloadServerWithModel(card.FileName);
                    }
                }
                catch (WebException wex)
                {
                    ...
                    else
                    {
                        ...
                        UnityEngine.Debug.LogError("[CAPCOM] Download network error: " + wex.Message);
                    }
                }
                catch (Exception ex)
                {
                    ...
                    UnityEngine.Debug.LogError("[CAPCOM] Download exception: " + ex.Message);
                }
            });
        }
    }
    ...
    [KSPAddon(KSPAddon.Startup.MainMenu, true)]
    public class CAPCOMUI : MonoBehaviour
    {
        ...
        private const string ControlLockID = "CAPCOM_InputLock";
        private const string ScrollLockID = "CAPCOM_ScrollLock";
        private bool isInputLocked = false;
        private bool isCameraLocked = false;

        private Font smoothFont;
        private GUIStyle capcomTextStyle;
        private GUIStyle pilotTextStyle;
        private GUIStyle capcomCardStyle;
        ...
        {
            "CAPCOM online. Standing by.",
            "CAPCOM on the loop. What are we launching today?",
            "CAPCOM here. Ready when you are.",
        ...
        private void OnAppLauncherReady()
        {
            ...
            {
                Texture2D iconTex = GameDatabase.Instance.GetTexture("CAPCOM/Textures/icon", false) ?? Texture2D.whiteTexture;
        ...
        private void InitStyles()
        {
            ...
            capcomTextStyle = new GUIStyle(GUI.skin.label)
            {
                font = smoothFont,
                wordWrap = true,
                richText = true,
            ...
            capcomCardStyle = new GUIStyle(GUI.skin.box)
            {
                font = smoothFont,
                padding = new RectOffset(10, 10, 8, 8),
        ...
        private void OnGUI()
        {
            ...
            windowRect = GUILayout.Window(845920, windowRect, DrawWindow, "CAPCOM", GUILayout.MinWidth(380), GUILayout.MinHeight(360));
        }
        ...
        private void DrawChatTerminal()
        {
            ...
            {
                bool isCapcom = msg.Sender == "CAPCOM";
                GUILayout.BeginVertical(isCapcom ? capcomCardStyle : pilotCardStyle);

                if (isCapcom)
                    GUILayout.Label("<color=#38ef7d><b>✦ CAPCOM</b></color>", capcomTextStyle);
                else
                    GUILayout.Label("<color=#4facfe><b>You</b></color>", pilotTextStyle);

                GUILayout.Space(2);
                string formattedText = MarkdownToRichText(msg.Text);
                GUILayout.Label(formattedText, isCapcom ? capcomTextStyle : pilotTextStyle);
                GUILayout.EndVertical();
            }

            if (IsBusy)
            {
                GUILayout.BeginVertical(capcomCardStyle);
        ...
        {
            GUILayout.Label("<b>Saved Conversations</b>", capcomTextStyle);
            GUILayout.Space(4);
            ...
            {
                GUILayout.Label("<color=#888888>No saved sessions found.</color>", capcomTextStyle);
            }
            else
            {
                for (int i = SavedSessions.Count - 1; i >= 0; i--)
                {
                    ...
                    GUILayout.Label("<size=10><color=#778899>" + s.CreatedDate + " • " + msgCount + " msgs</color></size>", capcomTextStyle);
                    GUILayout.EndVertical();
                }
            }
        ...
        private void DrawModelsManager()
        {
            ...
            foreach (var model in ModelManager.Models)
            {
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Label("<b>" + model.Name + "</b>", capcomTextStyle);
                GUILayout.Label("<color=#66ccff>" + model.VRAM + "</color>", capcomTextStyle, GUILayout.Width(95));
                GUILayout.EndHorizontal();

                GUILayout.Label("<size=11><color=#a0a8b0>" + model.Description + "</color></size>", capcomTextStyle);
                GUILayout.Space(6);
                ...
                else if (model.IsDownloaded)
                {
                    ...
                    if (isActive)
                    {
                        GUILayout.Label("<color=#55ff55><b>● LOADED</b></color>", capcomTextStyle, GUILayout.ExpandWidth(true));
                        
                        if (GUILayout.Button("Unload", GUILayout.Width(80), GUILayout.Height(24)))
                        {
                            CAPCOMPlugin.UnloadModel();
                        }
                    }
                    else
                    {
                        GUILayout.Label("<color=#888888>Installed</color>", capcomTextStyle, GUILayout.ExpandWidth(true));
                        if (GUILayout.Button("Load Model", GUILayout.Width(90), GUILayout.Height(24)))
                        {
                            CAPCOMPlugin.ReloadServerWithModel(model.FileName);
                        }
                    }
                    GUILayout.EndHorizontal();
                }
                ...
                {
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("<color=#ffaa55>Not Installed</color>", capcomTextStyle, GUILayout.ExpandWidth(true));
        ...
        private async void SendPrompt(string prompt)
        {
            ...
            string response = await AIService.QueryCAPCOM(CurrentSession.Messages);

            CurrentSession.Messages.Add(new ChatMessageData { Sender = "CAPCOM", Text = response });
            IsBusy = false;
            shouldScrollToBottom = true;
        ...
        private void OnDestroy()
        {
            ...
            if (toolbarButton != null && ApplicationLauncher.Instance != null)
            {
                ...
                toolbarButton = null;
																					...
																					// dump ksp data onto the local ai
    public static class AIService
    {
        private const string ServerUrl = "http://127.0.0.1:8080/v1/chat/completions";

        public static async Task<string> QueryCAPCOM(List<ChatMessageData> conversationHistory)
        {
            return await Task.Run(() =>
            {
                if (CAPCOMPlugin.serverProcess != null && CAPCOMPlugin.serverProcess.HasExited)
                {
                    return "[Server Crash]: llama-server.exe exited with code " + CAPCOMPlugin.serverProcess.ExitCode + ". Check GPU drivers or DLLs.";
                }
                ...
                try
                {
                    ...
                    string kspMasterInfo =
                        ...
                        "You are CAPCOM (Capsule Communicator), the technical flight specialist and mission companion running directly inside the player's KSP process.\n\n" +
                        ...
                        "- Identity: You are CAPCOM. Never identify as Qwen, Gemma, Alibaba, or Google.";
        ...
        public static string GetCurrentSceneContext()
        {
            ...
            if (HighLogic.LoadedScene == GameScenes.EDITOR)
            {
                if (EditorLogic.fetch != null && EditorLogic.fetch.ship != null)
                {
                    ...
                    sb.AppendLine(string.Format("• VAB/SPH Active Craft: '{0}' (Parts: {1}, Mass: {2:F2}t)", 
